...
- **Shared metadata**: continue to use `DWJOBFLW`, `DWJOBSCH`, `DWPRCLOG`, `DWJOBLOG`, `DWJOBERR`, etc., so no UI or reporting rewrites are needed.
...
| **Execution Engine** | Reads `DWJOBFLW.DWLOGIC`, `exec()` the stored Python code, calls `execute_job()` with params, and persists log/status rows. |
| **Job Queue Table (`DWPRCREQ`)** *(new)* | Lightweight table for “immediate” or ad-hoc run requests (regular or history). Scheduler polls it to trigger work outside Flask. |
...
```
Frontend ➜ Flask API ➜ metadata tables / DWPRCREQ ➜ Scheduler Service ➜ Execution Engine ➜ DWJOBLOG / DWPRCLOG / DWJOBERR
```
...
2. **Job Types**
   - `etl_flow`: executes generated code stored in `DWJOBFLW.DWLOGIC`.
...
3. **Triggers**
   - **Scheduled**: Derived from `DWJOBSCH` frequency columns. APScheduler trigger mapping mirrors `PKGDWPRC.GET_REPEAT_INTERVAL` logic:
   ...
   - **Immediate / Queue**: Poll `DWPRCREQ` (status `NEW`). When found, enqueue a one-off job run with supplied parameters, mark request as `RUNNING/COMPLETE/FAILED`.

4. **Dependencies**
   - Store parent-child links in `DWJOBSCH.DPND_JOBSCHID`.
...
5. **Concurrency / Workers**
   ...
   - Enforce per-mapref mutex if `DWPRCLOG` shows a job already `IP`, reproducing `check_job_already_running`.

6. **Checkpoint Compatibility**
   - `process_job_flow()`:
     1. Insert row into `DWPRCLOG` (status `IP`, params captured, session ID = new UUID).
     2. `exec()` job code to obtain callable(s); run inside `try/except`.
     3. On success, update `DWPRCLOG` (status `PC`, end timestamp), insert/refresh `DWJOBLOG` row using the row counts returned by the job code.
     4. On failure, update `DWPRCLOG` (status `FL`, error message) and insert `DWJOBERR`.
...
7. **History Runs**
   - `process_historical_data(mapref, start_date, end_date, truncate_flag)`:
     ...
     - Each iteration logs to `DWPRCLOG/DWJOBLOG`.

8. **Enable/Disable**
   - Instead of toggling DBMS scheduler jobs, mark `DWJOBSCH.SCHFLG` and add/remove APScheduler jobs.
...
9. **Stop Running Job**
   - Mark request in `DWPRCREQ` with action `STOP`.
   - Scheduler checks running workers; if job is `IP`, signal cancellation (set threading Event) and update `DWPRCLOG` status to `ST`. For long-running ETL code, we wrap `execute_job` to periodically check for cancel signals.

### 5. Data Model Additions / Changes
| Table | Change | Notes |
|-------|--------|-------|
| `DWPRCREQ` *(new)* | `request_id`, `mapref`, `request_type` (`IMMEDIATE`, `HISTORY`, `REPORT`), payload JSON, `status`, timestamps | Acts as durable queue between API and scheduler service. |
| `DWPRCLOG` | Add columns for `service_node`, `cancel_flag` (optional) | Helps in multi-node deployments. |
| `DWJOBSCH` | No schema change required; Python validates same business rules already enforced in PL/SQL. |

If schema changes are undesirable, `DWPRCREQ` can be replaced with an existing table (e.g., extend `DWPRCLOG`) but dedicated queue table keeps semantics clean.

### 6. Module Layout
```
backend/modules/jobs/
├── scheduler_service.py      # entry point; starts scheduler loop
├── scheduler_store.py        # queries DWJOBSCH/DWPRCREQ, maps to APScheduler jobs
...
- For immediate runs: endpoint inserts a row in `DWPRCREQ` (type `IMMEDIATE`) and returns request ID; scheduler picks it up asynchronously.

### 8. Report Module Integration
- Report mappings will store SQL/query + output spec in their own tables.
- Scheduler treats them as `report` jobs: loads mapping metadata, executes query via configured connection, writes output (file/table), and logs status using the same `DWPRCLOG/DWJOBLOG` pipeline.
...
5. Write migration notes for DB additions (`DWPRCREQ`, optional columns).
